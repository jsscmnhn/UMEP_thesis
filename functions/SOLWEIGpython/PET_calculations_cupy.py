...
def compute_metabolic_rate(mbody, age, ht, work, sex):
    metbf = 3.19 * mbody ** (3 / 4) * (1 + 0.004 * (30 - age) + 0.018 * ((ht * 100 / (mbody ** (1 / 3))) - 42.1))
    metbm = 3.45 * mbody ** (3 / 4) * (1 + 0.004 * (30 - age) + 0.010 * ((ht * 100 / (mbody ** (1 / 3))) - 43.4))
    met = np.where(sex == 1, metbm, metbf) + work
    return met


def compute_respiration_energies(ta, RH, met, p=1013.25):
    vps = 6.107 * 10. ** (7.5 * ta / (238. + ta))
    vpa = RH * vps / 100
    rtv = 1.44e-6 * met
    cair = 1.01 * 1000
    ...
    evap = 2.42e6
    erel = 0.623 * evap / p * (vpa - vpex) * rtv
    return eres + erel, vpa


def compute_geometry(mbody, ht, icl):
    adu = 0.203 * mbody ** 0.425 * ht ** 0.725
    facl = (-2.36 + 173.51 * icl - 100.76 * icl ** 2 + 19.28 * icl ** 3) / 100
    facl = np.clip(facl, 0, 1)
    fcl = 1 + 0.15 * icl
    rcl = icl / (6.45 * facl)
    y = np.where(icl <= 0.3, 0.1, np.where(icl <= 0.6, 0.5, (ht - 0.2) / ht))
    r2 = adu * (fcl - 1 + facl) / (2 * np.pi * ht * y)
    r1 = facl * adu / (2 * np.pi * ht * y)
    di = r2 - r1
    acl = adu * facl + adu * (fcl - 1)
    return adu, facl, fcl, rcl, y, r1, r2, di, acl


def compute_heat_transfer_coefficients(v, p=1013.25, po=1013.25):
    hc = (2.67 + 6.5 * v ** 0.67) * (p / po) ** 0.55
    return hc


def compute_energy_balance(ta, tmrt, tcl, tsk, hc, adu, facl, acl, feff, aeff, emsk=0.99, emcl=0.95, sigma=5.67e-8):
    tcl_k = tcl + 273.2
    tsk_k = tsk + 273.2
    tmrt_k = tmrt + 273.2

    rbare = aeff * (1 - facl) * emsk * sigma * (tmrt_k ** 4 - tsk_k ** 4)
    rclo = feff * acl * emcl * sigma * (tmrt_k ** 4 - tcl_k ** 4)

    cbare = hc * (ta - tsk) * adu * (1 - facl)
    cclo = hc * (ta - tcl) * acl

    return rbare + rclo, cbare + cclo


def PET_array(ta, RH, tmrt, v, mbody, age, ht, work, icl, sex):
    met = compute_metabolic_rate(mbody, age, ht, work, sex)
    ere, vpa = compute_respiration_energies(ta, RH, met)
    adu, facl, fcl, rcl, y, r1, r2, di, acl = compute_geometry(mbody, ht, icl)
    hc = compute_heat_transfer_coefficients(v)
    ...
    # Initialize tsk and tcl
    tsk = np.full_like(ta, 34.0)
    tcl = (ta + tmrt + tsk) / 3
    tcore = np.zeros_like(ta)
    wetsk = np.zeros_like(ta)

    # Skip full convergence loop, placeholder PET iteration
    for _ in range(3):
        rsum, csum = compute_energy_balance(ta, tmrt, tcl, tsk, hc, adu, facl, acl, feff, aeff)
        tsk = (hc * (tcl - ta) + emcl * sigma * ((tcl + 273.2) ** 4 - (tmrt + 273.2) ** 4) * feff) / htcl + tcl

    # PET final approximation
    tx = np.copy(ta)
    for _ in range(3):
        tex = 0.47 * tx + 21
        eres = cair * (tx - tex) * rtv
        vpex = 6.11 * 10 ** (7.45 * tex / (235 + tex))
        erel = 0.623 * evap / 1013.25 * (12 - vpex) * rtv
        ere_tx = eres + erel
        rsum, csum = compute_energy_balance(tx, tmrt, tcl, tsk, hc, adu, facl, acl, feff, aeff)
        ed = evap / (0.79e7) * adu * (1 - wetsk) * (12 - 6.11 * 10 ** (7.45 * tsk / (235 + tsk)))
        enbal = h + ed + ere_tx + csum + rsum + food
        tx = np.where(enbal > 0, tx - 0.1, tx + 0.1)
