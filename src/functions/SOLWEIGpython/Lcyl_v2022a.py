...
def Lcyl_v2022a(esky, sky_patches, Ta, Tgwall, ewall, Lup, shmat, vegshmat, vbshvegshmat, solar_altitude, solar_azimuth,
                rows, cols, asvf):
    ...
    for altitude in skyalt:
        ...
        Ldown[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta + 273.15) ** 4)) / np.pi) * steradian[
            patch_altitude == altitude] * np.sin(altitude * deg2rad)
        Lside[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta + 273.15) ** 4)) / np.pi) * steradian[
            patch_altitude == altitude] * np.cos(altitude * deg2rad)
        Lnormal[patch_altitude == altitude] = ((temp_emissivity * SBC * ((Ta + 273.15) ** 4)) / np.pi) * steradian[
            patch_altitude == altitude]
    ...
    Ldown, Lside, Lside_sky, Lside_veg, Lside_sh, Lside_sun, Lside_ref, \
        Least_, Lwest_, Lnorth_, Lsouth_ = patch_characteristics.define_patch_characteristics(solar_altitude,
                                                                                              solar_azimuth,
                                                                                              patch_altitude,
                                                                                              patch_azimuth, steradian,
                                                                                              asvf,
                                                                                              shmat, vegshmat,
                                                                                              vbshvegshmat,
                                                                                              Lsky_down, Lsky_side,
